        ...
        for pval in pvalues {
            ...
            match v {
                ...
                Entry::Occupied(mut entry) => {
                    ...
                    if self.phase == PaxosPhase::TWO && entry.get().ballot < pval.ballot {
...
mod tests {
    ...
    fn lame_duck() {
        ...
        assert_eq!(proposer.logger.lame_duck_superceded_by, Some(ballot_6_replica1()));
    ...
    fn stale_phase_one_response() {
        ...
        assert_eq!(proposer.logger.last_stale_proposal, Some(ballot_4_replica1()));
        assert_eq!(proposer.phase, PaxosPhase::ONE);
    }
    ...
    fn normal_acceptors_with_pvalues() {
        ...
        assert_eq!(proposer.proposals.get(&1), Some(&pval1));
        assert_eq!(proposer.proposals.get(&2), Some(&pval2));
        ...
        assert_eq!(proposer.proposals.get(&1), Some(&pval1));
        assert_eq!(proposer.proposals.get(&2), Some(&pval2));
        assert_eq!(proposer.proposals.get(&3), Some(&pval3));
    }
    ...
    fn pvalues_for_the_same_slot() {
        ...
        proposer.process_phase_1b_message(&REPLICA1, &ballot, &[pval1.clone(), pval2a.clone(), pval3b.clone()]);
        proposer.logger.assert_ok();
        assert_eq!(proposer.phase, PaxosPhase::ONE);
        assert_eq!(proposer.proposals.len(), 3);
        assert_eq!(proposer.proposals.get(&1), Some(&pval1));
        assert_eq!(proposer.proposals.get(&2), Some(&pval2a));
        assert_eq!(proposer.proposals.get(&3), Some(&pval3b));

        proposer.process_phase_1b_message(&REPLICA2, &ballot, &[pval1.clone(), pval2b.clone(), pval3a.clone()]);
        proposer.logger.assert_ok();
        assert_eq!(proposer.phase, PaxosPhase::TWO);
        assert_eq!(proposer.proposals.len(), 3);
        assert_eq!(proposer.proposals.get(&1), Some(&pval1));
        assert_eq!(proposer.proposals.get(&2), Some(&pval2b));
        assert_eq!(proposer.proposals.get(&3), Some(&pval3b));
    }
    ...
    fn pvalue_conflicts_are_logged() {
        ...
        assert_eq!(proposer.proposals.get(&1), Some(&pval1a));

        proposer.process_phase_1b_message(&REPLICA2, &ballot, &[pval1b.clone()]);
        assert_eq!(proposer.phase, PaxosPhase::TWO);
        assert_eq!(proposer.proposals.len(), 1);
        assert_eq!(proposer.proposals.get(&1), Some(&pval1b));
...
// this test requires mocking out message sending
...
//
    // TODO(rescrv):
    // - test that make_progress will send a message to every unaccepted acceptor
    // - test slots that fall outside the config range
    // - clean up redundancy
}
